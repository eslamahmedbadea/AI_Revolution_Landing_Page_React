// Hero Section Imgs
const PeopleImages = [
  // Img 1
  "https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=600&auto=format&fit=crop&q=60",

  // Img 2
  "https://plus.unsplash.com/premium_photo-1671656349322-41de944d259b?w=600&auto=format&fit=crop&q=60",

  // Img 3
  "https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=600&auto=format&fit=crop&q=60",
];

function HeroSection() {
  return (
    <section
      id="home"
      className="app-container pt-40 pb-40 overflow-hidden sm:overflow-visible"
    >
      <div className="grid sm:grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        {/* Left Content */}
        <div className="space-y-6">
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold leading-tight">
            <span className="gradient-text">Artficial Intelligence</span>
            <br />
            For a Smarter Future
          </h1>
          <p className="text-lg text-gray-600 max-w-lg">
            Discover how AI is transforming industries and improving lives
            worldwide. Our cutting-edge solutions harness the power of machine
            learning to solve complex problems.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 pt-4">
            <a href="#" className="btn-primary">
              Explore AI Solutions
            </a>
            <a href="#" className="btn-outline">
              Learn More
            </a>
          </div>
          {/* Left Content Imgs */}
          <div className="pt-4 flex items-center space-x-4">
            <div className="flex -space-x-2">
              {PeopleImages.map((img, i) => (
                <img
                  className="avatar"
                  key={i}
                  src={img}
                  alt={`User ${i + 1}`}
                />
              ))}
            </div>
            <p className="text-sm font-medium text-gray-600">
              Trusted by <span className="text-primary font-bold">10,000 </span>{" "}
              People worlwide
            </p>
          </div>
        </div>

        {/* Right Img */}
        <div className="relative w-full sm:h-80 md:h-150">
          <img
            src="https://images.unsplash.com/photo-1674027444474-e63f9d516f92?q=80&w=1632&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
            alt="hero-section"
            className="w-full h-full object-cover rounded-2xl"
          />
          <div className="absolute w-32 h-32 rounded-full bg-secondary/20 -bottom-6 -left-6"></div>
          <div className="absolute w-32 h-32 rounded-full bg-accent/20 -top-6 -right-6"></div>
        </div>
      </div>
    </section>
  );
}

export default HeroSection;
